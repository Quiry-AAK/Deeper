...
| `Player` | The player rig | `Prefabs/Player.prefab`, starts at (14, 8), the room centre |
| `Level` | `Grid` + `Floor` / `Walls` tilemaps | The 28×16 test room. **Room prefabs mount here.** |
...
| `F11` | Clear **all** enemies | One key for all four, not four clear keys |
...
clears **only** dummies, and `F12` is the last free function key.
